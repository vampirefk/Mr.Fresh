...
def getStopWord(stop_file):
    swlist = list()
    for line in open(stop_file, "r", encoding="utf-8").readlines():
        swlist.append(line.strip())
    print("加载停用词完成...")
    return swlist
...
if __name__ == "__main__":
    ...
    file = "test.txt"
    # 停用词文件
    stop_file = "./train_word_bag/stop_word.txt"
    keyWords = list()
    stop_words = getStopWord(stop_file)
    for line in open(file, "r", encoding="utf-8").readlines():
        ...
        for word in list(jieba.cut(str(title).replace(" ", ""), cut_all=False)):
            if word not in stop_words and word != " " and word != " ":
                kws.append(word)
        keyWords.append(str(id) + '\t' + ','.join(kws))
